...
export default function ProgramDetailClient({ program, related, whatsapp }: { program: Program; related: Program[]; whatsapp?: string }) {
  const hasCapacity = typeof program.totalSpots === "number" && typeof program.spotsLeft === "number" && program.totalSpots > 0;
  const fillPct = hasCapacity ? Math.round(((program.totalSpots! - program.spotsLeft!) / program.totalSpots!) * 100) : 0;
  const urgency = hasCapacity && program.spotsLeft! <= 3;
  ...
  const aboutLine = `This program is offered by ${owner.title} and the team will share the next steps after registration.`;
  const whatsappHref = whatsapp
    ? `https://wa.me/${whatsapp.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi, I have a question about ${program.title}.`)}`
    : null;
  ...
  return (
    <>
      ...
      <section className="relative h-[420px] overflow-hidden md:h-[520px]">
        <ImageWithFallback src={program.image} alt={program.title} className="absolute inset-0 h-full w-full object-cover" />
        ...
        <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-10 pt-8 md:px-8 md:pb-14">
          <div className="text-white">
            <Breadcrumbs
              items={[
                { label: "Home", href: "/" },
                { label: owner.shortTitle, href: owner.href },
                { label: program.title },
              ]}
            />
      ...
      <section className="relative bg-background pb-16">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 pt-12 md:px-8 lg:grid-cols-[1fr_360px]">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-10 font-display text-2xl font-extrabold text-brand-teal">Program Highlights</h2>
              ...
              <>
                ...
                <div className="mt-4 columns-2 gap-3 md:columns-3 [&>*]:mb-3">
                  {gallery.map((src, i) => (
                    ...
                    >
                      <ImageWithFallback src={src} alt={`${program.title} session ${i + 1}`} className="w-full object-cover" loading="lazy" />
                    </button>
                  ))}
                </div>
              </>
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-3xl bg-white p-6 shadow-lift">
              {program.price && <div className="font-display text-4xl font-black text-brand-orange">{program.price}</div>}
              <div className="text-xs font-semibold uppercase tracking-wider text-text-soft">program registration</div>

              <ul className="mt-5 space-y-3 text-sm text-brand-teal">
                {program.dates && <SidebarRow icon={<Calendar className="h-4 w-4" />} label="Dates" value={program.dates} />}
                {program.time && <SidebarRow icon={<Clock className="h-4 w-4" />} label="Time" value={program.time} />}
                {program.duration && <SidebarRow icon={<Hourglass className="h-4 w-4" />} label="Duration" value={program.duration} />}
                {program.location && <SidebarRow icon={<MapPin className="h-4 w-4" />} label="Location" value={program.location} />}
              </ul>

              {hasCapacity && <div className="mt-5 rounded-2xl bg-brand-mint p-4">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-brand-mint bg-white p-3 shadow-lift lg:hidden">
        <div className="flex items-center gap-3">
          <div>
            {program.price && <div className="font-display text-xl font-extrabold text-brand-orange">{program.price}</div>}
            {hasCapacity && <div className="text-[11px] text-text-soft">{program.spotsLeft} spots left</div>}
          </div>
          <Link
            href={`/register?program=${program.id}`}
